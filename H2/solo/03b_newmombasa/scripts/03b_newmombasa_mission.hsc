...
(global vehicle sergeant_warthog none)

;==============================================================
;=========== breadcrumbs improved navpoint triggers ===========
;==============================================================
(script dormant breadcrumbs_nav_points_03b
	; Tunnel entrance
	(if (= g_breadcrumb_nav_index 0)
		(begin
			(breadcrumbs_activate_team_nav_point_position "default" player 114.1 315.16 -3.87 "tunnel_entrance" 0.5)
			(sleep_until
				(or
					(volume_test_objects tv_e16_tunnel (players))
	...
	; Tunnel ramp when player has to exit from Scorpion
	(if (= g_breadcrumb_nav_index 1)
		(begin
			(breadcrumbs_activate_team_nav_point_position "default" player 244.26 261.56 -14.67 "tunnel_ramp" 0.3)
			(sleep_until
				(or
					(< (objects_distance_to_position (players) 244.26 261.56 -14.67) 2)
	...
	(if (= g_breadcrumb_nav_index 2)
		(begin
			(breadcrumbs_activate_team_nav_point_position "default" player 236.59 225.01 -12.65 "tunnel_hallway" 0.5)
			(sleep_until
				(or
					(< (objects_distance_to_position (players) 236.59 225.01 -12.65) 1)
					(> g_breadcrumb_nav_index 2)
				)
			)
			(breadcrumbs_deactivate_team_nav_point player "tunnel_hallway")
	...
	(if (= g_breadcrumb_nav_index 3)
		(begin
			(set sergeant_warthog (ai_vehicle_get_from_starting_location e18_mars_warthog0/warthog0))
			(breadcrumbs_activate_team_nav_point_object "default" player sergeant_warthog 0.1)
			(sleep_until
				(or
					(and
						(< (objects_distance_to_object sergeant_warthog (player0)) 6.0)
						(unit_in_vehicle (unit (player0)))
					)
					(> g_breadcrumb_nav_index 3)
				)
			)
			(breadcrumbs_deactivate_team_nav_point_object player sergeant_warthog)
			(sleep_until (> g_breadcrumb_nav_index 3))
		)
	)

	; Near the boulevard entrance
	(if (= g_breadcrumb_nav_index 4)
		(begin
			(breadcrumbs_activate_team_nav_point_position "default" player 286.7 140.8 -19.1 "navpoint_tunnel_exit" 0)
			(sleep_until
				(or
			...
			(breadcrumbs_deactivate_team_nav_point player "navpoint_tunnel_exit")
			(sleep_until (> g_breadcrumb_nav_index 4))
		)
	)
	
	; Near the boulevard exit
	(if (= g_breadcrumb_nav_index 5)
		(begin
			(breadcrumbs_activate_team_nav_point_flag "default" player e22_navpoint 0)
			(sleep_until (> g_breadcrumb_nav_index 5))
			(breadcrumbs_deactivate_team_nav_point_flag player e22_navpoint)
	...
	(if (= g_breadcrumb_nav_index 7)
		(begin
			(breadcrumbs_activate_team_nav_point_position "default" player 411.86 79.91 -9.49 "navpoint_door" 0.5)
			(sleep_until
				(or
					(< (objects_distance_to_position (players) 411.86 79.91 -9.49) 2)
					(> g_breadcrumb_nav_index 7)
				)
			)
			(breadcrumbs_deactivate_team_nav_point player "navpoint_door")
	...
	(if (= g_breadcrumb_nav_index 8)
		(begin
			(sleep_until (volume_test_objects tv_e23_entry_door (players)))
			(breadcrumbs_activate_team_nav_point_object "default" player scarab 10)
			(sleep_until
				(or
					(< (objects_distance_to_object (players) scarab) 15)
...
(script command_script cs_e23_cov_inf1_pilot0
	...
	(breadcrumbs_activate_team_nav_point_object "default" player (ai_get_object ai_current_actor) 0.5)
...
(script dormant mission_start
	...
	; Activate breadcrumb navpoints
	(wake breadcrumbs_nav_points_03b)
