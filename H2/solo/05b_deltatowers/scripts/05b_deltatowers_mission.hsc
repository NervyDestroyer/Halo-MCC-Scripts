...
(script dormant breadcrumbs_nav_points_05b
	; Starting door after first encounter
	(if (= g_breadcrumb_nav_index 0)
		(begin
			(breadcrumbs_activate_team_nav_point_position "default" player -1.61 -215.51 18.6 "nav_first_door" 0.5)
			(sleep_until
				(or
					(> g_breadcrumb_nav_index 0)
					(< (objects_distance_to_position (players) -1.61 -215.51 18.6) 3)
				)
			)
			(breadcrumbs_deactivate_team_nav_point player "nav_first_door")
		)
	)
	...
	(if (= g_breadcrumb_nav_index 0)
		(begin
			(sleep_until
				(or
					(volume_test_objects vol_tower1_exit_01 (players))
					(> g_breadcrumb_nav_index 0)
				)
			)
			(breadcrumbs_activate_team_nav_point_position "default" player -8.8 -191.5 19.3 "nav_central_platform" 0)

			(sleep_until
				(or
					(volume_test_objects vol_central_platform (players))
					(> g_breadcrumb_nav_index 0)
				)
			)
			(breadcrumbs_deactivate_team_nav_point player "nav_central_platform")
	...
	(if (= g_breadcrumb_nav_index 1)
		(begin
			(breadcrumbs_activate_team_nav_point_object "default" player gondola_01_switch_back 0.1)
			(sleep_until (> g_breadcrumb_nav_index 1))
			(breadcrumbs_deactivate_team_nav_point_object player gondola_01_switch_back)
			(sleep_until (> g_breadcrumb_nav_index 2))
		)
	)

	; Undersea elevator down
	(if (= g_breadcrumb_nav_index 3)
		(begin
			(breadcrumbs_activate_team_nav_point_object "default" player elev_under_switch_01 0.1)
			(sleep_until (> g_breadcrumb_nav_index 3))
			(breadcrumbs_deactivate_team_nav_point_object player elev_under_switch_01)
		)
	)
	...
	(if (= g_breadcrumb_nav_index 4)
		(begin
			(sleep_until
				(or
					(volume_test_objects vol_tunnel_01_entry (players))
					(> g_breadcrumb_nav_index 4)
				)
			)
			(breadcrumbs_activate_team_nav_point_position "default" player -31.25 -40.28 -51 "nav_underwater" 0.5)
			(sleep_until
				(or
					(> g_breadcrumb_nav_index 4)
					(< (objects_distance_to_position (players) -31.25 -40.28 -51) 3)
				)
			)
			(breadcrumbs_deactivate_team_nav_point player "nav_underwater")

			(set g_breadcrumb_nav_index 5)
		)
	)
	
	; Undersea elevator up
	(if (= g_breadcrumb_nav_index 5)
		(begin
			(breadcrumbs_activate_team_nav_point_object "default" player elev_up_switch_01 0.1)
			(sleep_until (> g_breadcrumb_nav_index 5))
			(breadcrumbs_deactivate_team_nav_point_object player elev_up_switch_01)
	...
	(if (= g_breadcrumb_nav_index 7)
		(begin
			(sleep_until
				(or
					(volume_test_objects vol_island_int_exit (players))
					(> g_breadcrumb_nav_index 7)
				)
			)
			(breadcrumbs_activate_team_nav_point_object "default" player gondola_02_switch_front 0.1)
			...
			(breadcrumbs_deactivate_team_nav_point_object player gondola_02_switch_front)
	...
	(if (= g_breadcrumb_nav_index 8)
		(breadcrumbs_activate_team_nav_point_object "default" player (list_get (ai_actors regret) 0) .5)
	)
)
					...
					(begin
						...
						(if (= (ai_vitality_pinned regret) FALSE)
							(begin
								...
								(breadcrumbs_deactivate_team_nav_point_object player regret_corpse)
								(object_destroy regret_corpse)
								(object_destroy regret_throne)
								(sleep 60)
								(ai_place regret 1)
...
;								(effect_new_on_object_marker "effects\objects\characters\prophet\regret_death" regret_throne "")
								...
								;(sleep 30)
;								(set regret_dead TRUE)
								(breadcrumbs_deactivate_team_nav_point_object player (list_get (ai_actors regret) 0))
...
;Mission stuff
	...
	(wake kill_volumes)
	(wake breadcrumbs_nav_points_05b)
