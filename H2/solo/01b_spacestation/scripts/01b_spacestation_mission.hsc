...
;=================================
	...
	(global short global_flavor_delay (* 20 tick_seconds))
...
(script static void save_1st_start
	...
	(set timer_flavor (max timer_flavor (* 10 tick_seconds)))
)
...
(script static short timer_flavor_set
	(random_range (* 15 tick_seconds) (* 30 tick_seconds))
)


(script static short timer_flavor_set_long
	(random_range (* 45 tick_seconds) (* 60 tick_seconds))
)
	...
	(sleep_until
		(cond
			(mark_flavor_disable	(begin
									...
									(set timer_flavor (max timer_flavor (* 5 30)))
									FALSE
								))
...
;==============================================================
(script dormant breadcrumbs_nav_points_01b
	 ; Start of the level
	(if (= g_breadcrumb_nav_index 0)
		(begin
			; Starting hallway opening
			(breadcrumbs_activate_team_nav_point_position "default" player 70.12 -16.12 5 "starting_hallway" 0.5)
			(sleep_until
				(or
					(< (objects_distance_to_position (players) 70.12 -16.12 5) 3)
					(> g_breadcrumb_nav_index 0)
				)
			)
			(breadcrumbs_deactivate_team_nav_point player "starting_hallway")
	
			; Wait for next navpoint to trigger
			(sleep_until (> g_breadcrumb_nav_index 0))
		)
	)
	
	; After initial combat encounter
	(if (= g_breadcrumb_nav_index 1)
		(begin
			; Door in stairs
			(breadcrumbs_activate_team_nav_point_position "default" player 71.56 -21.8 5 "door_stairs" 0.5)
			(sleep_until
				(or
					(< (objects_distance_to_position (players) 71.56 -21.8 5) 1)
					(> g_breadcrumb_nav_index 1)
				)
			)
			(breadcrumbs_deactivate_team_nav_point player "door_stairs")
	...
	(if (= g_breadcrumb_nav_index 2)
		(begin
			; Garage door
			(breadcrumbs_activate_team_nav_point_position "default" player 56.82 -36.74 3 "garage_door" 0.5)
			(sleep_until
				(or
					(< (objects_distance_to_position (players) 56.82 -36.74 3) 3)
					(> g_breadcrumb_nav_index 2)
				)
			)
			(breadcrumbs_deactivate_team_nav_point player "garage_door")
	...
	(if (= g_breadcrumb_nav_index 3)
		(begin
			; Entrance of second bay
			(breadcrumbs_activate_team_nav_point_position "default" player 65.89 -35.82 5 "door_forward" 0.5)
			(sleep_until
				(or
					(< (objects_distance_to_object (players) bsp2_door_forward) 2)
					(> g_breadcrumb_nav_index 3)
				)
			)
			(breadcrumbs_deactivate_team_nav_point player "door_forward")
	...
	; After the last group of enemies after the malta has exploded have been killed
	(if (= g_breadcrumb_nav_index 4)
		(begin
			; Doorway leadin into the next combat area
			(breadcrumbs_activate_team_nav_point_position "default" player 57.2 -58.87 3 "doorway_to_combat" 0.5)
			(sleep_until
				(or
					(< (objects_distance_to_position (players) 57.2 -58.87 3) 3)
					(> g_breadcrumb_nav_index 4)
				)
			)
			(breadcrumbs_deactivate_team_nav_point player "doorway_to_combat")
	...
	; After Athens Explode event
	(if (= g_breadcrumb_nav_index 5)
		(begin
			; Reach the Armory
			(breadcrumbs_activate_team_nav_point_position "default" player 32.1 -53.0 0.5 "armory_door_left" 0.5)
			(breadcrumbs_activate_team_nav_point_position "default" player 36.3 -48.8 0.5 "armory_door_right" 0.5)
			(sleep_until
				(or
					(< (objects_distance_to_position (players) 32.1 -53.0 0.5) 3)
					(< (objects_distance_to_position (players) 36.3 -48.8 0.5) 3)
					(> g_breadcrumb_nav_index 5)
				)
			)
			(breadcrumbs_deactivate_team_nav_point player "armory_door_left")
			(breadcrumbs_deactivate_team_nav_point player "armory_door_right")
	...
	(if (= g_breadcrumb_nav_index 6)
		(begin
			...
			(breadcrumbs_activate_team_nav_point_position "default" player 14.4 -60.8 5.0 "navpoint_1" 0.5)
			(sleep_until
				(or
					(< (objects_distance_to_position (players) 14.4 -60.8 5.0) 2)
					(> g_breadcrumb_nav_index 6)
				)
			)
	...
	(if (= g_breadcrumb_nav_index 7)
		(begin
			(breadcrumbs_activate_team_nav_point_position "default" player 25.46 -80.75 5 "dck_doorway" 0.5)
			(sleep_until
				(or
					(< (objects_distance_to_position (players) 25.46 -80.75 5) 2)
					(> g_breadcrumb_nav_index 8)
				)
			)
			(breadcrumbs_deactivate_team_nav_point player "dck_doorway")
	...
	(if (= g_breadcrumb_nav_index 8)
		(begin
			(sleep_until
				(or
					(volume_test_objects tv_dck_floor (players))
					(> g_breadcrumb_nav_index 8)
				)
			)
			(breadcrumbs_activate_team_nav_point_object "default" player bsp3_door_backward_2 .2)
			(sleep_until
				(or
					(< (objects_distance_to_object (players) bsp3_door_backward_2) 2)
					(> g_breadcrumb_nav_index 8)
				)
			)
			(breadcrumbs_deactivate_team_nav_point_object player bsp3_door_backward_2)
	...
	(if (= g_breadcrumb_nav_index 9)
		(begin
			(breadcrumbs_activate_team_nav_point_object "default" player elv_elevator_control_1 .1)
			(sleep_until
				(or
					(> (device_group_get elv_elevator_position) 0)
					(> g_breadcrumb_nav_index 9)
				)
			)
			(breadcrumbs_deactivate_team_nav_point_object player elv_elevator_control_1)
	...
	(if (= g_breadcrumb_nav_index 10)
		(begin
			(sleep_until (volume_test_objects tv_lvr_enter (players)))
			(breadcrumbs_activate_team_nav_point_position "default" player -7.6 -3.5 -4.4 "navpoint_3" 0)

			(sleep_until (< (objects_distance_to_position (players) -7.6 -3.5 -4.4) 2))
...
;	(save_elv_mid)
	...
	(sleep_until (volume_test_objects_all tv_elevator (ai_actors elv_cov_elevator))	1 (* 20 30))

	(device_set_position elv_elevator 1)
	(set g_breadcrumb_nav_index 9)

	(sleep_until (> (device_group_get elv_elevator_position) 0) 1 (* 120 30))
...
;	(sleep_until (> (scenery_get_animation_time carrier_01) 0) 1)
	...
	(sleep_until (volume_test_objects tv_lvr_back (players)) 1 (* 15 30))
