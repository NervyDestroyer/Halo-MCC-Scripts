	...
	(if (not (>= s_objcon_court 40))
		(begin
			(if (not (chud_breadcrumbs_using_revised_nav_points))
				...
				(f_bcs_blip_position -74.10 57.44 17 "court_nav" blip_destination)
			)
			(sleep_until (>= s_objcon_court 40) 5)
			(if (not (chud_breadcrumbs_using_revised_nav_points))
				(f_unblip_flag fl_court_waypoint_1)
				(f_bcs_unblip_position "court_nav")
			)
		)
	)
...
(script dormant bcs_nav_to_far
	(f_bcs_blip_position 6.21 -132 6.1 "far_path_nav" blip_destination)
	(sleep_until (volume_test_players tv_air_far_start) 1)
	(f_bcs_unblip_position "far_path_nav")
)
	...
	(if (not (>= s_objcon_return 30))
		(begin
			(if (not (chud_breadcrumbs_using_revised_nav_points))
				...
				(f_bcs_blip_position -64.17 11.42 12.21 "return_nav" blip_destination)
			)
			(sleep_until (>= s_objcon_return 30) 5)
			(if (not (chud_breadcrumbs_using_revised_nav_points))
				(f_unblip_flag fl_return_waypoint_1)
				(f_bcs_unblip_position "return_nav")
			)
		)
	)
	...
	(if (not (>= s_objcon_garage 10))
		(begin
			(if (not (chud_breadcrumbs_using_revised_nav_points))
				...
				(f_bcs_blip_position -95.54 56.35 15.45 "garage_nav" blip_destination)
			)
			(sleep_until (>= s_objcon_garage 10) 5)
			(if (not (chud_breadcrumbs_using_revised_nav_points))
				(f_unblip_flag fl_garage_waypoint_1)
				(f_bcs_unblip_position "garage_nav")
			)
		)
	)
