...
;starting breadcrumbs navpoints
(script dormant bcs_nav_start
	(hud_breadcrumbs_activate_team_nav_point_flag player elev_navpoint2  0.55)
	(sleep_until (<= (objects_distance_to_flag (players) elev_navpoint2) 2))
	(hud_breadcrumbs_deactivate_team_nav_point_flag player elev_navpoint2)
	(wake bcs_nav_warthog)
)

(script dormant bcs_nav_warthog
	(hud_breadcrumbs_activate_team_nav_point_object player (ai_vehicle_get_from_starting_location sq_elev_warthog1/warthog1) "warthog_nav" 0.8)
	(sleep_until 
		(or
	...
	(hud_breadcrumbs_deactivate_team_nav_point_object player "warthog_nav")
)

(script dormant bcs_nav_elev_navpoint
	(hud_breadcrumbs_activate_team_nav_point_flag player elev_navpoint 0.55)
	(sleep_until (>= s_cavern_obj_control 1) 1)
	(hud_breadcrumbs_deactivate_team_nav_point_flag player elev_navpoint)
)
...
(script dormant sc_elev_navpoint
	...
	(if (and
			(= s_cavern_obj_control 0)
			(not (hud_breadcrumbs_using_revised_nav_points))
		)
		(begin	
...
(script dormant sc_round_bcs_nav
	(sleep_until (volume_test_players tv_round_save) 1)
	
	(if (= s_exit_obj_control 0) 
		(begin	
			(hud_breadcrumbs_activate_team_nav_point_position player -127.34 261.45 0.21 "round_bcs_nav" 0.55)
			(sleep_until (volume_test_players tv_exit_oc3) 1)
			(sleep 1)
			(hud_breadcrumbs_deactivate_team_nav_point_position player "round_bcs_nav")
		)
	)
)
