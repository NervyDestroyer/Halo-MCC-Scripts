...
(script dormant 110_highcharity_cortana
	...
	; breadcrumbs improved navpoint
	(hud_breadcrumbs_activate_team_nav_point_flag player nav_reactor_switch 0.05)
...
(script dormant nav_ph
	...
	(if (<= g_ph_obj_control 3)
		(begin
			...
			(hud_breadcrumbs_activate_team_nav_point_flag player nav_hab_01 0)
		)
	)
)
...
(script dormant nav_hab
	...
	(if (= g_hab_obj_control 0)
		(begin
			...
			(hud_breadcrumbs_deactivate_team_nav_point_flag player nav_hab_01)
		)
	)
	...
	(if (<= g_pr_obj_control 3)
		(begin
			...
			(sleep_until
				(or
					(>= g_pr_obj_control 4)
					(<= (objects_distance_to_flag (players) nav_pr_01) 2)
				)
			1)
...
(script dormant bcs_nav_trapped_corty
	(sleep_until (>= g_re_obj_control 5) 1)
	(hud_breadcrumbs_activate_team_nav_point_flag player trapped_corty_flag g_nav_offset)
	(sleep_until g_cortana_rescued 1)
	(hud_breadcrumbs_deactivate_team_nav_point_flag player trapped_corty_flag)
)
