...
(script dormant PathA_Start	
	...
	(hud_breadcrumbs_deactivate_team_nav_point_object player "nav_warthog")
...
(script dormant ring_nav_exit_end
	...
	(if (hud_breadcrumbs_using_revised_nav_points)
...
(script static void deactivate_all_navs
	(Hud_deactivate_team_nav_point_flag player teleport_path_a_player2)
	(Hud_deactivate_team_nav_point_flag player nav_intro_exit)
	(Hud_deactivate_team_nav_point_flag player nav_patha_exit)
	(Hud_deactivate_team_nav_point_flag player nav_cella_exit)
	(Hud_deactivate_team_nav_point_flag player nav_locka_elevator)
...
;*********************************************************************;
;Breadcrumbs improved Nav Point Scripts
;*********************************************************************;

(script dormant bcs_nav_warthog
	(sleep 360)
	(hud_breadcrumbs_activate_team_nav_point_object player (ai_vehicle_get_from_starting_location patha_hog/warthog01) "nav_warthog" 1.0)
	(sleep_until (any_players_in_vehicle))
	(hud_breadcrumbs_deactivate_team_nav_point_object player "nav_warthog")
)

(script dormant start_bcs_nav_exit
	(deactivate_all_navs)
	(hud_breadcrumbs_activate_team_nav_point_flag player teleport_path_a_player2 0.55)
	(sleep_until 
				(or
					(volume_test_players path_a_game_save01)
					(<= (objects_distance_to_flag (players) teleport_path_a_player2) 5)
				)
	)
	(Hud_breadcrumbs_deactivate_team_nav_point_flag player teleport_path_a_player2)
	(deactivate_all_navs)
)

(script dormant bcs_nav_cella_exit
	(Hud_breadcrumbs_activate_team_nav_point_flag player nav_cella_exit 0)
	(sleep_until (volume_test_players vol_lock_a01_start) 1)
	(Hud_breadcrumbs_deactivate_team_nav_point_flag player nav_cella_exit)
	(wake bcs_nav_next_door)
)

(script dormant bcs_nav_next_door
	(Hud_breadcrumbs_activate_team_nav_point_flag player teleport_lock_a01b 0.55)
	(sleep_until (<= (objects_distance_to_flag (players) teleport_lock_a01b) 2))
	(Hud_breadcrumbs_deactivate_team_nav_point_flag player teleport_lock_a01b)
	(wake bcs_nav_elevator)
)

(script dormant bcs_nav_elevator
	(print "bcs_nav_elevator here")
	(Hud_breadcrumbs_activate_team_nav_point_flag player nav_locka_elevator 0)
	(sleep_until (> (device_get_position tower1_elevator) 0) 1)
	(Hud_breadcrumbs_deactivate_team_nav_point_flag player nav_locka_elevator)	
)

(script dormant bcs_nav_elevator_back
	(Hud_breadcrumbs_activate_team_nav_point_object player tower1_elevator_switch "nav_elevator_switch" 0.8)
	(sleep_until (< (device_get_position tower1_elevator) 1) 1)
	(hud_breadcrumbs_deactivate_team_nav_point_object player "nav_elevator_switch")
)

(script dormant bcs_nav_before_hornet
	(Hud_breadcrumbs_activate_team_nav_point_flag player nav_patha_exit 0)
	(sleep_until (<= (objects_distance_to_flag (players) nav_patha_exit) 6))
	(Hud_breadcrumbs_deactivate_team_nav_point_flag player nav_patha_exit)

	(wake hornet_nav_start)
)

(script dormant bcs_nav_after_hornet
	(hud_breadcrumbs_activate_team_nav_point_position player 188.33 41.63 24.46 "nav_after_hornet" 0.55)
	(sleep_until (volume_test_players vol_cell_c_towerbase) 1)
	(hud_breadcrumbs_deactivate_team_nav_point_position player "nav_after_hornet")
)

(script dormant bcs_lock_c_nav_start01
	(Hud_breadcrumbs_activate_team_nav_point_flag player nav_lockc_start 0)
	(sleep_until (volume_test_players vol_cell_c_brute_spawn) 1)
	(Hud_breadcrumbs_deactivate_team_nav_point_flag player nav_lockc_start)
)

(script dormant beach_bcs_nav_exit
	(deactivate_all_navs)
	(sleep_until 
		(or
			(volume_test_players vol_patha_choppers)
	...
	(if (not (volume_test_players vol_beach_hog_area))
		(begin
			(hud_breadcrumbs_activate_team_nav_point_flag player nav_intro_exit 0)
			(sleep_until 
				(and
					(volume_test_players vol_beach_hog_area)
					(any_players_in_vehicle)
			...
			5)
			(deactivate_all_navs)
			;(hud_breadcrumbs_deactivate_team_nav_point_flag player nav_intro_exit)
		)
	)
)
;*********************************************************************;
